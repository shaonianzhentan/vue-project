...
export default class {
   ...
   */
  clipboard(text) {
      const id = 'unique-id-clipboard-copyText'
   ...
   */
  debounce(fn, wait) {
      const cache = this.debounce.prototype.cache || {}
      const fnKey = fn.toString()
      const timeout = cache[fnKey]
   ...
   */
  throttle(fn, delay) {
      const cache = this.throttle.prototype.cache || {}
      const fnKey = fn.toString()
      const timeout = cache[fnKey]
      // 如果正在进行中，则不做处理
  ...
  sum(arr, isSubtract = false) {
      ...
      for (let i = 0; i < arr.length; i++) {
          ...
          // 获取小数点后位数
          const len = this.pointLength(value)
          if (point < len) point = len
      }
      // 取整放大倍数
      const maxPoint = Math.pow(10, point)
      // 累加结果字段
